...
function install_load_action() {
	...
	if($test_val == 'install_stage') {	
		...
		$db_passwd;
		$db_name;
		if(!($db_host = check_arg("db_host", ARG_TYPE_SESSION)) || !($db_port = check_arg("db_port", ARG_TYPE_SESSION))
			|| !($db_user = check_arg("db_user", ARG_TYPE_SESSION)) || !($db_passwd = check_arg("db_passwd", ARG_TYPE_SESSION))
			|| !($db_name = check_arg("db_name", ARG_TYPE_SESSION)))
			return 0;

		if(!test_connection_db($db_host, $db_port, $db_user, $db_passwd, $db_name))
			return 0;
			
		if(!core_add_user($dash_user, $dash_passwd))
			return 0;
			
		$file = fopen("include/settings.php", "a");
		fwrite($file, "<?php \n");
		fwrite($file, "/* Generated by NeweraPaaS, please do not modify */\n");
		fwrite($file, '$site_title' ."='NeweraPaaS Cloud';\n");
		fwrite($file, '$site_footer'."='Newera';\n");

		fwrite($file, '$db_agent' ."='mysql';\n");
		fwrite($file, '$db_host=' ."'$db_host';\n");
		fwrite($file, '$db_port=' ."'$db_port';\n");
		fwrite($file, '$db_user=' ."'$db_user';\n");
		fwrite($file, '$db_passwd'."='$db_passwd';\n");
		fwrite($file, '$db_name=' ."'$db_name';\n");
	
		fwrite($file, '$theme_active='."'initializr';\n");
		fwrite($file, '$modules'."=array('home', 'identity');\n");
...
function install_load_content() {
	...
	if($inst_stage == 1) {
		...
		$form_params['error']  = "Cannot initialize database";
		...
		$form = core_generate_form($form_params);
		return "<div class='well'><legend><h1>NeweraPaaS Installation</h1></legend>$form</div>";
   } else if($inst_stage == 4) {
		$message = "Other service like networking and project namespaces can be created from the dashboard itself.<br>";
   	$data = "<div class='well'><legend><h1>Welcome to NeweraPaaS Cloud</h1></legend>$message</div>";
   	return $data;	
   }
}
